...
func (server *ServerAgent) Start() {
	...
	fmt.Println("Création des joueurs")
	...
	fmt.Println("Lancement des tables")
	for i := range server.tables {
		server.wg.Add(1)
		go server.tables[i].Start()
	}
	fmt.Println("juste avant le wait")
	server.wg.Wait()

	fmt.Println("Lancement des parties")
	for i := 0; i < server.nbGames; i++ {
		...
		for turn := 0; turn < 4; turn++ {
			...
			for j := 0; j < server.nbTables; j++ {
				fmt.Println("On envoie ", turn, " dans le channel")
				server.c[j] <- turn
				server.wg.Add(1)
			}
			server.wg.Wait()
		...
		fmt.Println("Avant le wait")
		server.wg.Wait()
	}

	fmt.Println("Parties terminées, fermeture des tables")
